...
export interface GodotNode {
    ...
    child_node_ids: Record<string, number>; // node ids to index
}
...
export function serializeGodotScene(structured_content: GodotScene): string {
    ...
    if (structured_content.nodes && structured_content.root_node_id) {
        ...
        if (rootNode) {
            serializeNode(output, rootNode, structured_content, nodePathMap);
        }
    }
    ...
    for (const [, connection] of sortedConnections) {
        ...
        if (connection.unbinds !== undefined) {
...
function serializeNode(output: string, node: GodotNode, scene: GodotScene, nodePathMap: Map<string, number>): string {
    ...
    // Write node ID as metadata
    output += `metadata/patchwork_id = "${node.id}"\n\n`;

    // sort the children by index
    const sortedChildIds = Object.keys(node.child_node_ids).sort((a, b) => node.child_node_ids[a] - node.child_node_ids[b]);
    for (const childId of sortedChildIds) {
        const childNode = scene.nodes[childId];
        if (childNode) {
            output = serializeNode(output, childNode, scene, nodePathMap);
        }
    }
...
function getNodePath(nodeId: string, scene: GodotScene): string {
